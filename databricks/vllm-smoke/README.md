...
verify that the pinned vLLM stack, Cachet wheel install, model download, GPU
runtime, Cachet `DocumentKVConnector` `KVTransferConfig`, and output artifact
path work in a target workspace.
...
Prepared mode writes `dataset_source=prepared`, the sizing values, the Cachet
