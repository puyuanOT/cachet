...
from restaurant_kv_serving.benchmark_runner import BenchmarkEngineRequest, BenchmarkGeneration
...
class OpenAICompatibleCompletionEngine:
    ...
    def _post_json(self, payload: Mapping[str, Any]) -> Any:
        request = Request(
            urljoin(self.config.base_url.rstrip("/") + "/", self.config.endpoint.lstrip("/")),
