...
successful. That validation must identify the cache-arm request itself by its
prompt-token total, rather than warmup requests or a later baseline request that
benefits from ordinary SGLang prefix-cache reuse. Manual handoff inputs remain
supported when callers already have a validated SGLang handoff plus
