...
and the current deterministic no-thinking cache-hit quality failure in
...
`temperature=0.0`, but the live prompt still fails quality for both baseline
and cache arms. A publishable live run must make the Qwen3/SGLang
