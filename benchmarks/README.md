...
| [`sglang/`](sglang/) | SGLang benchmark status and live smoke folders | Generated-handoff Databricks smokes now reach partial external page hydration; latest blocker is per-batch `prior_hash` metadata for split HiCache page binding; live latency and quality benchmark pending |
...
[`sglang/`](sglang/), including the latest batch prior-hash metadata blocker at
