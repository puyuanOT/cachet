...
| Quality metric in this table | Prepared-suite answer-found rate from the forced-256 outputs |
| Current evidence | [`appendix/current-q4-q8-vllm-qwen3-4b-g5-a10g/`](appendix/current-q4-q8-vllm-qwen3-4b-g5-a10g/) |

The quality columns are smoke-suite answer-found checks, not full benchmark
accuracy. Forced-256 generations are verbose, so strict exact-match is tracked
in evidence but is not used as the primary quality column here.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Baseline, no precomputed KV | 8k | 2.166 | 2.737 | 14.507 | 15.080 | 1.00 | 1.00 | 1.00 | 1.00 |
| Baseline, no precomputed KV | 16k | 6.047 | 6.644 | 20.728 | 21.324 | 1.00 | 1.00 | 1.00 | 1.00 |
| Baseline, no precomputed KV | 32k | 16.622 | 16.971 | 35.361 | 35.714 | 1.00 | 1.00 | 1.00 | 1.00 |
| Cachet + vanilla KV | 8k | 0.389 | 0.422 | 12.730 | 12.752 | 1.00 | 1.00 | 1.00 | 1.00 |
| Cachet + vanilla KV | 16k | 0.432 | 0.595 | 15.196 | 15.468 | 1.00 | 1.00 | 1.00 | 1.00 |
| Cachet + vanilla KV | 32k | 0.677 | 1.122 | 19.705 | 20.000 | 1.00 | 1.00 | 1.00 | 1.00 |
| Cachet + KV Packet | 8k |  |  |  |  |  |  |  |  |
...
| Document KV payload | P50 TTFT (s) | P95 TTFT (s) | P50 TTC (s, 256 tokens) | P95 TTC (s, 256 tokens) | Answer-found rate | Strict EM | Cache footprint | Peak GPU memory | CPU RSS / host RAM | Notes |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| bf16 | 0.539 | 0.646 | 15.224 | 15.336 | 0.00 | 0.00 | 9.83 GB |  |  | Quality failure under FP8 runtime KV: measured outputs did not contain expected answers |
| Q8 (`fp8_e5m2`) | 0.432 | 0.595 | 15.196 | 15.468 | 1.00 | 0.00 | 4.92 GB |  |  | Default document KV precision |
...
Peak GPU memory, GPU utilization, CPU RSS, and host RAM were not sampled in
these runs. vLLM server logs report 16.32 GiB available KV-cache memory,
237,728 GPU KV-cache tokens, and 7.25x maximum concurrency for
32,768-token requests after loading the 4-bit Qwen3-4B model.
...
| Storage tier | P50 TTFT (s) | P95 TTFT (s) | P50 TTC (s, 256 tokens) | P95 TTC (s, 256 tokens) | Cache footprint | Peak GPU memory | CPU RSS / host RAM | Notes |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| RAM |  |  |  |  |  |  |  | Not measured under the current protocol |
| Disk | 0.432 | 0.595 | 15.196 | 15.468 | 4.92 GB |  |  | Current default |
...
| Hardware | P50 TTFT (s) | P95 TTFT (s) | P50 TTC (s, 256 tokens) | P95 TTC (s, 256 tokens) | Cache footprint | Peak GPU memory | CPU RSS / host RAM | Notes |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| AWS g5/A10G, `g5.8xlarge` | 0.432 | 0.595 | 15.196 | 15.468 | 4.92 GB |  |  | Current default |
...
| Serving platform | P50 TTFT (s) | P95 TTFT (s) | P50 TTC (s, 256 tokens) | P95 TTC (s, 256 tokens) | Cache footprint | Peak GPU memory | CPU RSS / host RAM | Notes |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| vLLM | 0.432 | 0.595 | 15.196 | 15.468 | 4.92 GB |  |  | Current default |
