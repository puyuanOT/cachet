...
| [`sglang/`](sglang/) | SGLang benchmark status and live smoke folders | Generated-handoff Databricks smokes reached partial external page hydration; latest blocker is attach-time `get_hash_str` patchability for per-batch `prior_hash` metadata; live latency and quality benchmark pending |
...
[`sglang/`](sglang/), including the latest attach-time hash tracking blocker at
