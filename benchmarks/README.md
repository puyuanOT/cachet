...
| [`sglang/`](sglang/) | SGLang benchmark status and live smoke folders | Generated-handoff Databricks smokes reached a full 175-token external cache hit covering a generated Qwen-chat prefix through the chat-completions path with deterministic no-thinking controls, explicit Triton/PyTorch backend controls, a minimal no-thinking request body, and the model-quality canary moved after live checks; latest blocker is the failed Qwen3/SGLang baseline, cache-arm, and post-live-check canary quality gate; live latency and quality benchmark pending |
...
[`sglang/`](sglang/), including the latest canary-after-cache-hit quality
failure at
