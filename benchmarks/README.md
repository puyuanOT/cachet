...
This directory is the public benchmark appendix for Cachet. It is organized
like a paper table: one main comparison table first, then ablations. It does
not invent missing numbers. A blank numeric cell means not measured yet; not
zero.

The committed appendix evidence at
[`appendix/existing-results/`](appendix/existing-results/) does not match the
fixed main-table configuration below. Treat it as prior evidence, not as the
source for the empty main-table cells. Rows marked `not measured yet` are not
benchmarked yet under the stated configuration.
