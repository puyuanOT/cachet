...
| Use premium package branding | Implemented | The distribution remains the transitional `document-kv-cache`; the product/import brand is Cachet with `cachet.<module>` imports plus `cachet-*` and `document-kv-*` CLI aliases. | Rename the GitHub repository to `puyuanOT/cachet` before public publication. |
