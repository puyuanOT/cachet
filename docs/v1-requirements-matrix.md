...
- Release-evidence validation over the current target benchmark, storage, and
  ...
  benchmark via the `compatibility_benchmark` role. The post-#442 current-wheel
  complete g5-enriched strict bundle validates with 24 artifacts after replacing
  its benchmark/status sidecars with the current
  `872615985402004`/`566743786103032` evidence, including PR #442 evidence for
  the core restaurant-alias removal, and carrying `legacy_migration_evidence`
  for the removed restaurant facade. The bundled artifacts include the release evidence
