...
| Document quality and latency metrics | Release-gated | `benchmarks.py`, `benchmark_runner.py`, `openai_compatible.py`, and `release_evidence.py` validate TTFT, time-to-completion, throughput, answer quality, and cache-vs-baseline comparisons. The public benchmark folder now defines the current Qwen3-4B 4-bit-weight + Q8-document-KV protocol as cold disk-to-GPU document-KV hydrate for Cachet latency rows. The committed rows in `benchmarks/appendix/current-q4-q8-vllm-qwen3-4b-g5-a10g/` are warm-prefix canary evidence: Cachet Q8 P50/P95 TTFT is 0.389s/0.422s at 8k, 0.432s/0.595s at 16k, and 0.677s/1.122s at 32k, with prewarm-only Cachet KV loads and zero measured-request KV loads. Those warm-prefix rows are retained for audit only and do not populate the main cold-hydrate latency table. The visible quality table is a prepared-suite smoke check that reports answer-found containment beside strict EM, not official Biography/HotpotQA/MusiQue/NIAH scores. The public score table also reserves full-dataset columns for LongBench v2 and RULER once their dataset runners and evidence are available. The bf16 document-KV precision ablation is measured separately and recorded as a warm-prefix quality failure under the FP8 runtime KV setting. | Re-run, re-bundle, and refresh the current appendix benchmark folder whenever benchmark code, runtime pins, connector behavior, package wheel identity, serving platform, or document-KV precision changes. Cold-hydrate latency rows must use per-request `cache_salt` isolation and at least 512 repeats per prepared input at the fixed request parallelism. |
