...
- [`docs/repo-map.md`](../repo-map.md) for the repository layout.
- [`docs/evidence-policy.md`](../evidence-policy.md) for what belongs in
...
- [`docs/repo-map.md`](../repo-map.md) explains the top-level folders and the
...
SGLang paths, see [`docs/native-engine-integration.md`](../native-engine-integration.md).
...
monolithic full-prefix control, use
`build_post_rope_transformers_kv_chunk_generator` and omit those two options.
...
cachet-benchmark-handoff-bundles \
  ...
  --generator-factory document_kv_cache.transformers_generator:build_pre_rope_transformers_kv_chunk_generator \
  --cache-method vanilla_prefill \
...
python -m document_kv_cache.benchmark_plan \
  ...
  --benchmark-handoff-generator-factory document_kv_cache.transformers_generator:build_post_rope_transformers_kv_chunk_generator \
...
python -m document_kv_cache.vllm_smoke \
  ...
  --max-tokens 100 \
  --benchmark-handoff-generator-factory document_kv_cache.transformers_generator:build_pre_rope_transformers_kv_chunk_generator \
  --benchmark-handoff-chunk-per-document \
  --benchmark-handoff-cache-method vanilla_prefill \
...
The current helper is a baseline/provider bring-up gate only. Cachet's built-in
...
a clean baseline first, then flushes SGLang's in-memory prefix cache through
`/flush_cache` before the handoff-backed cache arm. That keeps baseline quality
independent while preventing ordinary prefix-cache state from warming the
cache-hit validation. The smoke flushes again before the model-quality canary,
so the canary tests fresh Qwen3 generation rather than prefix-cache state left
by the live probes. Use `--no-flush-cache-before-cache-arm` or
`--no-flush-cache-before-canary` only for diagnostics that intentionally compare
against previous ordering.
...
python -m document_kv_cache.release_bundle \
  ...
  --pr-evidence-json docs/release-ops/pr-evidence/release-provenance.json \
  --legacy-migration-evidence-json docs/release-ops/evidence/legacy-migration/current/legacy-migration-evidence.json \
...
`docs/release-ops/evidence/dependency-freshness/current/`: direct package metadata pins must
